...
RUN TURBO_TELEMETRY_DISABLED=1 pnpm exec turbo run build --filter=@aster/discovery \
    && pnpm --filter=@aster/discovery --prod deploy --legacy /out
...
EXPOSE 3500
STOPSIGNAL SIGTERM
HEALTHCHECK --interval=3s --timeout=2s --start-period=20s --retries=6 CMD ["node", "--input-type=module", "--eval", "try { const r = await fetch('http://127.0.0.1:3500/health/ready', { signal: AbortSignal.timeout(1000), redirect: 'error' }); await r.body?.cancel(); process.exitCode = r.status === 200 ? 0 : 1; } catch { process.exitCode = 1; }"]
ENTRYPOINT ["node"]
