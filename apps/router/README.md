...
Status: implemented locally for P04-R01/R04/R05/R08/R10. This package builds schemas; it is not an Apollo Router process. Runtime, trusted propagation, traces and partial-failure acceptance remain the next Phase 04 slice.
...
| Errors | Identity expected mutation outcomes use `IdentityOutcome` payloads and correlation IDs. Unexpected owner failures use sanitized GraphQL errors. Router-level mapping and transport failure behavior are not verified by composition. |
...
| Nullability | `me`, `profile`, `activeProfile` and `title` can be null; `profiles` and `titles` are non-null roots. A failure on a non-null root can null the whole response. `ViewerAndTitle` deliberately uses nullable roots for the next runtime partial-failure test. |
...
[Composition evidence](../../evidence/phase-04/composition.txt) records deterministic output, negative cases, committed-baseline isolation and candidate checks. No Router query plan, network trust, request telemetry, browser playback or hosted compatibility is inferred from a schema check. Roll back this tooling/artifact slice without touching product data or running containers.
