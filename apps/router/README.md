...
Phase 04 is released. This package composes Identity, Catalog, Playback and the implemented Phase 08 Engagement owner. The separate pinned Apollo Router executes the generated supergraph with private owner credentials and bounded telemetry. [Phase 04 evidence](../../evidence/phase-04/README.md), [Playback evidence](../../evidence/phase-07/README.md).
...
Both root schema commands first build Identity, Catalog, Playback, Engagement and this tooling through declared workspace dependencies. Each owner prints its executable Federation schema in a bounded local child process; no service, PostgreSQL, Docker, GraphOS registry, credential or introspection request is needed. `schema:check` is read-only. `schema:update` explicitly regenerates the seven files under [infra/router/generated](../../infra/router/generated/manifest.json), then verifies them. Commit the resulting artifact set together, not a hand-edited subset.

The generated set contains four subgraph SDLs, the public API SDL, Router-ready supergraph SDL and a format-version-1 manifest with exact tool versions, file hashes, field/entity ownership and fourteen known-operation hashes. Routing URLs are internal service names, not public endpoints. Composition uses the existing approved Apollo 2.14.4 and GraphQL 16.14.2 pins; [ADR-0003](../../docs/adr/0003-federation.md) and [ADR-0014](../../docs/adr/0014-apollo-federation-license-policy.md) remain applicable.
...
The finite router-trust-init creates four per-owner Router credentials plus three separate owner-read credentials: Playback-to-Catalog and Engagement-to-Identity/Playback. Router mounts only its four keys; each private-read key is shared solely by its two participants. Valid files are reused on restart; insecure or missing files fail startup. Never print credentials. For rotation, stop affected consumers, validate the exact disposable volume labels and absence of foreign attachments, replace only those volumes and restart consumers together. Retain PostgreSQL/media. The guarded whole-project reset recognizes all seven disposable trust volumes but is not a key-only rotation command. [Playback trust and operations](../../services/playback/README.md).
...
Router health on internal 8088 means the process is running; it does not promise all subgraphs are available. Internal metrics use 9091. Owners retain independent health/readiness and database authority. Router startup requires Catalog, Playback and trust initialization, not optional Identity/Engagement. Starting the entire runtime/full profile also starts those owners; targeting Router alone supports anonymous playback without them. The disposable proof uses this unchanged base dependency graph. Identity queries fail independently while that owner is absent.
