...
- The P01-R07 PostgreSQL connectivity checkpoint exact-pins `pg@8.23.0` and development-only `@types/pg@8.23.1` behind `@aster/postgres`. The adapter bounds capacity and deadlines, destroys a connection after abort, timeout, or unknown protocol state, emits only the released finite dependency telemetry vocabulary, and exposes no vendor type, general SQL seam, schema, migration, repository, or transaction policy. Real PostgreSQL interoperability remains P01-R09.
