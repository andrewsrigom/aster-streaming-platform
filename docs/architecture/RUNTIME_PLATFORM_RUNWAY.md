...
The exact 2.2.4 package schedules its pending-request maintenance timer from a zero `throttledUntil` value, which can pass a negative epoch-sized delay to Node after real broker traffic and emit `TimeoutNegativeWarning`. A repository patch clamps every nonpositive schedule to KafkaJS's existing one-second pending-request interval. This preserves the queue's intended polling behavior, is applied through pnpm's hashed `patchedDependencies`, is copied into every frozen Docker build context, and remains under KafkaJS's MIT terms. Discovery runtime acceptance rejects either negative/overflow timer warning so packaging drift fails visibly.
