...
Serialize policy read/merge/write/readback with an exclusively created private `control/publication-access.lock` object (`If-None-Match: *`), relying on ADR-0022's single POSIX writer. Delete only that control object after successful readback. Cancellation, crash or ambiguous write retains the barrier; there is no expiring lease that could admit a replacement while an old write remains in flight. Existing media reads continue. Operator recovery must stop all publishers and restart the exact private writer before inspecting/removing the stale barrier and retrying full verification. No media or database data is removed. This local protocol is not a hosted multi-writer guarantee; Phase 14 must verify its chosen storage's atomicity/fencing. The existing complete local bundle receives a one-time recognized-legacy-policy restriction after integrity/reference checks. Never silently replace unknown retained policy.
