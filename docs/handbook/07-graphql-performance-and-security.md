...
Aster's released Phase13 proof applies those rules to the complete generated
graph: 12 lists, 10 entity returns and 5 contributors derive exact owner, scope,
resolution and bound records during composition. Exact current documents
through Router observed TitleDetail2, SearchTitles5, HomePublic7 and
ContinueWatching7 PostgreSQL queries across all participating owners. These are
constant-count checks at fixed workloads, not hosted latency objectives. The
proof compares
the complete observed owner set with the exact budget; it filters only the
shared SQL's explicitly named readiness and fixture-background statements and
never narrows observations to expected owner keys. The
[Phase13 release record](../../evidence/phase-13/release.md) links the protected,
review, merge and exact-main acceptance.
