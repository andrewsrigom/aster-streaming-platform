...
Phase acceptance checkpoint (base f28c442): [all twelve local requirement rows](../evidence/phase-06/acceptance.md) are covered. Resolve the earlier orphan-storage assessment through ADR-0026's explicit local retention boundary: clean disposable scratch; retain immutable private/public objects for checked recovery; do not claim automatic S3 garbage collection. Hosted lifecycle/budget/fencing belongs to P14-R11. No source/media/SQL change or repeated encode is needed. Check acceptance prose and current registry audit, publish one coherent candidate, require protected exact-head CI and confirmation, squash without bypass, verify post-merge, then start Phase 07. Until those gates finish, P06-R01 remains IN_PROGRESS and the full Phase 00–14 goal remains active.
