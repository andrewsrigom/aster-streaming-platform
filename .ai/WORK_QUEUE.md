...
The optional-profile checkpoint now passes real metrics, Collector outage isolation/recovery, bounded degraded shutdown and exact nine-service reset. Focused config/Identity/platform/CI checks pass 20/34/29/22. CI image/profile coverage is added. Exact clean source `38801ce` passes Docker-only/occupied-port/reset acceptance, 49/49 uncached tasks and audit. PR 18 run `33046068184` passes all six jobs. A stale-runway review finding is corrected in one documentation-only batch; final confirmation/exact-head CI and release remain P01-R10 work.
