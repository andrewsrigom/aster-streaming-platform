...
const name = "aster-discovery-test-" + randomUUID();
const label = "aster.discovery.fixture=" + name;
...
try {
  ...
  const result = await execute(
    process.execPath,
    ["services/discovery/dist/test/integration/projection-postgres.js", String(port)],
    {
      cwd: root,
      timeout: 60000,
  ...
  for (const id of ids) {
    ...
    assert.equal(info.Config.Labels["aster.discovery.fixture"], name);
  ...
  process.stdout.write(
    JSON.stringify({
      event: "discovery_fixture_cleaned",
      remaining: 0,
...
if (primaryError) {
  throw new Error("Discovery integration failed.", { cause: primaryError });
}
