...
const name = "aster-discovery-cache-test-" + randomUUID();
const label = "aster.discovery.cache.fixture=" + name;
...
try {
  ...
  const result = await execute(
    process.execPath,
    ["services/discovery/dist/test/integration/cache-redis.js", String(port)],
    {
      cwd: root,
      timeout: 20_000,
      maxBuffer: 1024 * 1024,
  ...
  const ids = (
    ...
    .split("\n")
    .filter(Boolean);
  assert.ok(ids.length <= 1, "Unexpected Discovery cache fixture ownership.");
  for (const id of ids) {
    ...
    assert.equal(info.Config.Labels["aster.discovery.cache.fixture"], name);
  ...
  process.stdout.write(
    JSON.stringify({
      event: "discovery_cache_fixture_cleaned",
      remaining: 0,
...
if (primaryError) {
  throw new Error("Discovery cache integration failed.", { cause: primaryError });
}
