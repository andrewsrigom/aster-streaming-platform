...
if [ "$1" = "compose" ]; then
  ...
  if [ "$1" = "config" ] && [ "$2" = "--volumes" ]; then
    printf '%s\\n' postgres-data broker-data storage-data prometheus-data identity-router-trust catalog-router-trust playback-router-trust playback-catalog-trust engagement-router-trust engagement-identity-trust engagement-playback-trust engagement-catalog-trust discovery-router-trust discovery-catalog-trust
    exit 0
  fi
...
test("accepts all fourteen reviewed volumes but refuses a fifteenth before teardown", async (t) => {
  assert.equal(reviewedVolumes.length, 14);
  const complete = await runReset(t, { scenario: "all-volumes" });
  assert.equal(complete.code, 0, complete.stderr);
  assert.match(complete.stdout, /containers=1 networks=1 volumes=14/u);
  ...
  assert.match(overflow.stderr, /more than fourteen Aster volumes/u);
