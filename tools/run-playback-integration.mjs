...
const name = "aster-playback-test-" + randomUUID();
const label = "aster.playback.fixture=" + name;
...
try {
  ...
  const result = await execute(
    process.execPath,
    ["services/playback/dist/test/integration/sessions-postgres.js", String(port)],
    {
      cwd: root,
      timeout: 60000,
      maxBuffer: 1024 * 1024,
  ...
  for (const id of ids) {
    ...
    assert.equal(info.Config.Labels["aster.playback.fixture"], name);
  ...
  process.stdout.write(
    JSON.stringify({
      event: "playback_fixture_cleaned",
      remaining: 0,
...
if (primaryError) {
  throw new Error("Playback integration failed.", { cause: primaryError });
}
