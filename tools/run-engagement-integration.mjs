...
const name = "aster-engagement-test-" + randomUUID();
const label = "aster.engagement.fixture=" + name;
...
try {
  ...
  for (const id of ids) {
    ...
    assert.equal(info.Config.Labels["aster.engagement.fixture"], name);
...
if (primaryError) {
  throw new Error("Engagement integration failed.", { cause: primaryError });
}
