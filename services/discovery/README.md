...
Snapshot refresh and rebuild export use independent process-local Catalog breakers with the same 30-second/four-sample/50% policy, five-second open interval and one half-open probe. An open snapshot circuit does not block export or Playback. Rejection adds no queue and creates no data; only an already valid active projection may remain serviceable under its existing lease. Finite result/transition metrics identify the fixed operation class. [ADR-0041](../../docs/adr/0041-operation-scoped-circuit-breakers.md) defines the state machine.
