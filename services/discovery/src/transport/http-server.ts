...
export interface DiscoveryHttpServerOptions {
...
export interface DiscoveryHttpServer {
...
export function createDiscoveryHttpServer(
  options: DiscoveryHttpServerOptions,
): DiscoveryHttpServer {
  ...
  return Object.freeze({
    listen(signal: AbortSignal): Promise<void> {
      ...
      if (stopping || signal.aborted) {
        return Promise.reject(new Error("Discovery HTTP startup was cancelled."));
      }
      startup = new Promise<void>((resolve, reject) => {
        ...
        const onError = (): void => {
          cleanup();
          binding.abort();
          reject(new Error("Discovery HTTP listener could not start."));
        };
        const onAbort = (): void => {
          cleanup();
          binding.abort();
          reject(new Error("Discovery HTTP startup was cancelled."));
        };
