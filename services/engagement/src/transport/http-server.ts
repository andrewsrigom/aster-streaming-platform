...
export interface EngagementHttpServerOptions {
...
export interface EngagementHttpServer {
...
export function createEngagementHttpServer(
  options: EngagementHttpServerOptions,
): EngagementHttpServer {
  const adapter = createExpressHttpAdapter({
    healthSnapshotProvider: options.health,
  ...
  return Object.freeze({
    listen(signal: AbortSignal): Promise<void> {
      ...
      if (stopping || signal.aborted) {
        return Promise.reject(new Error("Engagement HTTP startup was cancelled."));
      }
      startup = new Promise<void>((resolve, reject) => {
        ...
        const onError = (): void => {
          cleanup();
          binding.abort();
          reject(new Error("Engagement HTTP listener could not start."));
        };
        const onAbort = (): void => {
          cleanup();
          binding.abort();
          reject(new Error("Engagement HTTP startup was cancelled."));
        };
