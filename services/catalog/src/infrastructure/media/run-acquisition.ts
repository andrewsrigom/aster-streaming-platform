...
export async function runMediaAcquisition(
  ...
  let pendingCheck: Promise<CatalogMediaRequest> | undefined;
  ...
  function check(): Promise<CatalogMediaRequest> {
  ...
  try {
    ...
    const result = await ports.acquisitions.complete(attempt.id, source.original, current);
    ...
    return {
      ...result,
      evidence: { downloadElapsedMs: source.elapsedMs, peakMemory: source.peakMemory },
    };
  } catch (error) {
    finished = true;
    clearTimeout(timer);
