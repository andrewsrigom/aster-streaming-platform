...
Status: Phases 03–10 are released. Catalog provides the rights-aware editorial application, operator CLI, published-only Federation v2 owner, private Playback/Engagement/Discovery reads, the rights-safe public-title cache and [one locally published film](../../evidence/phase-06/publication.md).
...
Each request has its own DataLoader, at most 128 cached IDs and 20 IDs per set-based query; duplicates/missing entities preserve input order. When `ASTER_CATALOG_CACHE_ENABLED=true`, valid `Title` entity reads use the rights-safe Redis cache from [ADR-0037](../../docs/adr/0037-rights-safe-catalog-cache.md). Every positive hit first checks the current PostgreSQL publication/rights/version fence. Concurrent cold absence checks share a bounded owner-fence read only within the same request time and rights-use policy before writing a negative marker. Its Redis expiry and independently validated cache time both cap reuse at ten seconds; missing, future or older times are malformed and the exact key is deleted. Redis lease acquisition atomically replaces wrong-type, non-expiring or longer-than-two-second contaminated keys while preserving finite holders inside the requested coordination window. The Redis-side bounded read classifies oversized or non-string values before returning bytes and retains accepted bytes until fatal UTF-8 decoding. Invalid encoding, invalid envelopes and control-byte strings reach exact malformed-key recovery without resetting the shared connection. Cache values contain only bounded already-public projections; browse and Playback authority remain uncached. HTTP is POST-only JSON with Cache-Control: no-store, ignores viewer/operator headers and never issues cookies. The shared HTTP adapter bounds the body (32 KiB in the verified harness); subgraph preflight bounds source to 16 KiB, tokens to 2048, expanded fields to 128, depth to 10, aliases to 16, input nodes/depth to 256/8 and weighted cost to 4096. Lists use their real declared bounds. Eight requests may execute concurrently; the process-global token bucket allows a burst of 64 and refills eight/second. One three-second deadline and client disconnect cancel owner work; saturation returns 503, rate exhaustion 429. Input failures use sanitized INVALID_INPUT/LIMIT_EXCEEDED, dependency failure UNAVAILABLE and cancellation CANCELLED, with correlation IDs.
...
pnpm catalog:cache-integration
~~~

Integration uses the existing pinned PostgreSQL and Redis images and deletes only each new labelled, tmpfs-only fixture. PostgreSQL proof covers both lock orderings of publish/dispute, rollback after outbox writes, replay, receipt expiry, reserved retirement capacity, forbidden privileges, Unicode/artwork history, migration round-trips and real CLI processes including stdin deadline. Public-read proof additionally covers expiry before LIMIT, keyset stability, a one-query entity batch over real HTTP/PostgreSQL, immediate takedown, query plans and cancellation during a confirmed SQL lock wait. Cache proof covers expiry, conditional lease ownership, atomic compare-delete, concurrent positive and negative cold-key bursts, warm reuse, cross-instance coordination and Redis outage bypass. [Evidence](../../evidence/phase-03/README.md) distinguishes local acceptance from remote release; [Phase 10 evidence](../../evidence/phase-10/README.md) records the released cache, stale-serving and operation-admission slices. No film is approved or playable by these tests.
