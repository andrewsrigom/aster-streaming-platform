...
Phase 08 adds migration 0003 for the owner relay lease/index and restricted claim/ack functions, plus a separate local relay login. The current initializer accepts contiguous versions through 3; older initializers reject that ledger. Drain before upgrading. No product backfill or request-role widening occurs. [Event operations](../../engagement/EVENT_DELIVERY.md) defines deployment, key retention, finite waits and empty/idle-only downgrade. [Real SQL acceptance](../../../evidence/phase-08/events-postgres.txt) passes; Kafka/runtime candidate release remains pending.
...
The finite `identity-init` command uses one admin connection, nonblocking advisory lock `(42781, 2)`, a ten-second overall deadline and bounded SQL waits. It accepts only absent or contiguous ledger versions 1 through 3, rejects unknown versions and never resets retained data or reapplies successful SQL. It creates `aster_identity_local` and the separate relay login with fixed synthetic local credentials and restricted role membership; existing passwords are not overwritten. Runtime readiness rejects admin credentials and missing product columns. This local-only initializer is not a hosted migration service.
