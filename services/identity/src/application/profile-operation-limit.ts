import type { createIdentityProfiles, ProfileMutationKind } from "./profiles.js";
import type { createIdentitySessions } from "./sessions.js";
import type { ProfileMutationResult, ProfileRequest, ProfileResult } from "./profile-ports.js";
...
export function createRateLimitedIdentityProfiles(options: RateLimitedIdentityProfilesOptions) {
  const admitted = async <T>(
    ...
    try {
      const admissionId = options.digest(`${operation}\0${admissionIdentity}`);
  ...
  const mutate = async (
    ...
    request: ProfileRequest,
    input: unknown,
    invoke: () => Promise<ProfileResult<ProfileMutationResult>>,
    ...
    try {
      const replay = await options.profiles.probeMutationReplay(kind, request, input);
      if (replay.status !== "completed") {
        return replay;
      }
      if (replay.value.kind === "replay") {
        return { status: "completed", value: replay.value.result };
      }
      return await admitted(
        "profile_mutation",
        request,
        replay.value.accountId,
        replay.value.admissionIdentity,
        invoke,
      );
    } catch {
      return { status: cancelled() ? "cancelled" : "unavailable" };
  ...
  const select = async (request: ProfileRequest, profileId: unknown) => {
    const cancelled = (): boolean => request.signal.aborted;
    if (cancelled()) {
    ...
    try {
      ...
      return { status: cancelled() ? "cancelled" : "unavailable" } as const;
    }
  };
  ...
  return Object.freeze({
    ...
    create: (request: ProfileRequest, input: unknown) =>
      mutate("create", request, input, () => options.profiles.create(request, input)),
    update: (request: ProfileRequest, input: unknown) =>
      mutate("update", request, input, () => options.profiles.update(request, input)),
    delete: (request: ProfileRequest, input: unknown) =>
      mutate("delete", request, input, () => options.profiles.delete(request, input)),
    select,
  });
}
