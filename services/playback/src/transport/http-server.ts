...
export interface PlaybackHttpServerOptions {
...
export interface PlaybackHttpServer {
...
export function createPlaybackHttpServer(options: PlaybackHttpServerOptions): PlaybackHttpServer {
  ...
  return Object.freeze({
    listen(signal: AbortSignal): Promise<void> {
      ...
      if (stopping || signal.aborted) {
        return Promise.reject(new Error("Playback HTTP startup was cancelled."));
      }
      startup = new Promise<void>((resolve, reject) => {
        ...
        const onError = (): void => {
          cleanup();
          binding.abort();
          reject(new Error("Playback HTTP listener could not start."));
        };
        const onAbort = (): void => {
          cleanup();
          binding.abort();
          reject(new Error("Playback HTTP startup was cancelled."));
        };
