...
The local P01-R09 laboratory provides `pnpm integration:core`, `pnpm integration:broker` and `pnpm integration:storage` on Linux/WSL. It checks real PostgreSQL/Redis/Identity recovery and shutdown, Kafka delivery/offset replay, and S3 streaming/checksum/multipart cancellation using separate disposable Docker projects. The default project is unchanged; these tests require pinned host Node/pnpm. Collector/Prometheus and complete P01-R09 acceptance remain pending; the Docker-only application command belongs to P01-R10. See [integration operation and cleanup](docs/operations/LOCAL_DEVELOPMENT.md#real-postgresqlredis-integration) and [current evidence](evidence/phase-01/real-integration.txt).
