...
The local P01-R09 laboratory provides `pnpm integration:core`, `pnpm integration:broker`, `pnpm integration:storage` and `pnpm integration:telemetry` on Linux/WSL. It checks real PostgreSQL/Redis/Identity recovery and shutdown, Kafka delivery/offset replay, S3 streaming/checksum/multipart cancellation, and Collector/Prometheus export/scrape/recovery using disposable Docker projects. The default project is unchanged; these tests require pinned host Node/pnpm. Complete P01-R09 acceptance remains pending; the Docker-only application command belongs to P01-R10. See [integration operation and cleanup](docs/operations/LOCAL_DEVELOPMENT.md#real-postgresqlredis-integration) and [current evidence](evidence/phase-01/real-integration.txt).
